...
/*random intercept gamma model*/
proc glimmix data=GinaL empirical;
  nloptions maxiter=250 technique=newrap;
  class id;
  model activity=night cognitiveimpairment|depressed adl / s ddfm=satterth d=gamma;
  random intercept / subject=id type=un;
  output out=glimmixout_g pred(  blup ilink)=PredVal
                        pred(noblup ilink)=PredVal_PA /*PA:population-averaged*/
                        residual=residual;
run;
/*generalised chi-square / DF = 0.74*/
/*estimated intraclass correlation = 0.47 ~ 0.42 (?? or 0.39) in GEE(gamma)*/


/*plotting hierarchical gamma model by CognitiveImpairment*/
goptions reset=all;
proc gplot data=glimmixout_g;
  ...
  title h=2 "GLMM (linearisation): pred. noct. activity (seconds) by group, 95% CI";
run;
quit;
...
goptions reset=all;
proc gplot data=glimmixout_g;
  plot predval_PA*night=ADL_bin / haxis=axis1 vaxis=axis2 legend=legend1;
  symbol1 c=black v=none i=stdm2jt l=1 w=2 mode=include;
  symbol2 c=red v=none i=stdm2jt l=2 w=2 mode=include;
  axis1 label=(h=1 'Night') value=(h=1) minor=none;
  axis2 label=(h=1 A=90 "Predicted") value=(h=1) order=(0 to 3000 by 1000) minor=none;
...
proc gplot data=Glimmixout_g;
  ...
  title2 h=1 "linearisation (proc glimmix)";
run;
quit;
...
proc transpose data=Glimmixout_g out=residualw prefix=resid;
    by id;
    id night;
    var residual;
run;
