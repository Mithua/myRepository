...
proc glimmix data=GinaL empirical;
  ...
  class id;
  model activity=night cognitiveimpairment|depressed adl / s ddfm=satterth d=nb;
  random intercept / subject=id type=un;
  output out=glimmixout_nb pred(  blup ilink)=PredVal
                          pred(noblup ilink)=PredVal_PA /*PA:population-averaged*/
                          residual=residual;
run;
/*generalised chi-square / DF = 1.00*/
/*estimated intraclass correlation = 0.47*/


/*plotting hierarchical negative binomial model by CognitiveImpairment*/
goptions reset=all;
proc gplot data=glimmixout_nb;
